...
* **1.1** Используйте 4 пробела для табуляции.
...
* **3.1.3** Предпочтительней не объявлять типы для константант и переменных ?????????? Prefer not declaring types for constants or variables if they can be inferred anyway.
...
* **3.1.8** Не заключайте в скобки сравнения.
...
* **3.1.9** По возможности используйте значение `enum` вместе указания полной строки.
...
* **3.1.10** Не используйте сокращения для методов классов.
...
* **3.1.11** Предпочтительно не использовать `self.`, кроме случаев где это необходимо.

* **3.1.12** При написании методов подумайте, предназначен ли метод для переопределения или нет. Если нет, пометьте его как final, но имейте в виду, что это предотвратит перезапись метода в целях тестирования. В общем, методы final приводят к сокращению времени компиляции, поэтому рекомендуется использовать их, когда это применимо.

* **3.1.13** Если у вас есть функция, которая не принимает аргументов, не имеет побочных эффектов и возвращает какой-либо объект или значение, лучше вместо этого использовать вычисляемое свойство.
